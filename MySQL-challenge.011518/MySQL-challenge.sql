...
SELECT f.title, COUNT(*) AS `Rental Freq.`
FROM film f
JOIN inventory inv
...
GROUP BY f.title
ORDER BY `Rental Freq.` DESC;

# 7f. Write a query to display how much business, in dollars, each store brought in.
SELECT inv.store_id, SUM(pay.amount) AS `Rental Sales`
FROM inventory inv
JOIN rental re
ON re.inventory_id = inv.inventory_id
JOIN payment pay
...
SELECT cat.name, SUM(pay.amount) AS `Sales`
FROM category cat
JOIN film_category fcat
ON fcat.category_id = cat.category_id
JOIN inventory inv
